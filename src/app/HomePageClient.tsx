...
export default function HomePageClient() {
  const [markers, setMarkers] = useState<
    Awaited<ReturnType<typeof fetchHomeProjectMarkers>>
  >([]);
  const [grantMarkers, setGrantMarkers] = useState<
    Awaited<ReturnType<typeof fetchHomeGrantMarkers>>
  >([]);
  const [issueMarkers, setIssueMarkers] = useState<
    Awaited<ReturnType<typeof fetchHomeWatchdogMarkers>>
  >([]);
  const [homeStats, setHomeStats] = useState<
    Awaited<ReturnType<typeof fetchHomeStats>> | null
  >(null);

  // optional: show skeleton state if you want
  ...
  }, [markers.length, grantMarkers.length, issueMarkers.length, homeStats]);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const data = await fetchHomeProjectMarkers();
      ...
      try {
        const data = await fetchHomeGrantMarkers();
        if (!cancelled) setGrantMarkers(data);
      } catch (error) {
        logOnce("home-grants", "Home globe: funding marker query failed", error);
      }

      try {
        const data = await fetchHomeWatchdogMarkers();
        if (!cancelled) setIssueMarkers(data);
      } catch (error) {
      ...
      try {
        ...
        if (!cancelled) setHomeStats(data);
      } catch (error) {
  ...
  return (
    <main className="flex min-h-[calc(100vh-3.5rem)] flex-col gap-12 px-6 py-10">
